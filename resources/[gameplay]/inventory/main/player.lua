...
if IsDuplicityVersion() then 
    ...
    Functions.GetPlayer = function(src)
        ...
        if not Players[src] then 
            ...
            Functions.DBQuery("SELECT * FROM `player_inventory` WHERE `identifier`='"..identifier.."'",function(result)
            ...
            self.Functions.Save = function()
                Functions.DBQuery("INSERT INTO `player_inventory` (`identifier`, `inventory`) VALUES('"..identifier.."','"..json.encode(self.inventory).."') ON DUPLICATE KEY UPDATE `inventory` = '"..json.encode(self.inventory).."' ")
                return true
            end
        end
